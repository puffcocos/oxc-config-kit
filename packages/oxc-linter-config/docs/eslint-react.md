...
이 문서는 `oxc-linter-config`에서 활성화하지 않은 ESLint React 관련 규칙 목록입니다.

아래 규칙들은 `oxc-linter-config` 설정 구성 시점에 OXC 린터에서 아직 구현되지 않은 상태였습니다.
향후 구현이 완료되면 설정에 포함할지 검토가 필요합니다.
